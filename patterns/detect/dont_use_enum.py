...
class DontUseEnumDetector(Detector):
    def __init__(self):
        self.p = regex.compile(r'\w+\s+(enum|assert)\s*=\s*')
        self.p_method = regex.compile(r'\w+\s+(enum|assert)\s*\(.*\)\s*{')
    ...
    def match(self, linecontent: str, filename: str, lineno: int, **kwargs):
        ...
        if self.p_method.search(strip_line):
            self.bug_accumulator.append(
                BugInstance('NM_FUTURE_KEYWORD_USED_AS_METHOD_IDENTIFIER', priorities.MEDIUM_PRIORITY, filename, lineno,
                            'Nm: Use of identifier that is a keyword in later versions of Java')
            )